...
(select bidder_id,
case
when bid_status='won' then rank()over(partition by bidder_id)
when bid_status='lost' then 0
end win_count
from ipl_bidding_details)
select bidder_id,sum(win_count) win_count,count(bidder_id) no_of_bids,round((sum(win_count)/count(bidder_id))*100,1) percentage_win
from t
group by bidder_id
order by round((sum(win_count)/count(bidder_id))*100,1) desc
;
...
select stadium_id,stadium_name,city,count(stadium_id) no_of_matches from ipl_stadium ipls
join  ipl_match_schedule iplms
using(stadium_id)
group by stadium_id
order by stadium_id;
...
end)/count(stadium_id))*100,2) percentage_wins
from ipl_match iplm
join  ipl_match_schedule iplms
using(match_id)
join ipl_stadium ipls
using(stadium_id)
group by stadium_id
order by stadium_id
;
...
select bid_team,team_name,count(bid_team) total_bids 
from ipl_bidding_details ipld
join ipl_team iplt
on ipld.bid_team=iplt.team_id
group by bid_team
order by bid_team;
...
select distinct substr(win_details,1,8)win,
case
when win_details like '%CSK%' then 1
 ...
 ELSE 8
 END TEAM_id_from_win_details
from  ipl_match
order by  TEAM_id_from_win_details
;
...
#6.	Display total matches played, total matches won and total matches lost by the team along with its team name.
 select bid_team,team_name,count(bid_team) total_matches_played,
	sum(case
	when bid_status='won' then 1
	end
	)total_matches_won,
sum(case
	when bid_status='lost' then 1
	end
	)total_matches_lost
from ipl_bidding_details iplbd
join ipl_team iplt
on iplbd.bid_team=iplt.team_id
group by bid_team
order by bid_team
;
...
where team_id=(select team_id from ipl_team where remarks like '%mi%') and player_role='bowler';
###############################################################################################################################
...
select team_id,no_of_players
from
(select team_id,count(player_role) no_of_players from ipl_team_players
where player_role like '%all%'
group by team_id)t
where no_of_players>4
...
select player_name,player_role,(select team_name from ipl_team iplt where iplt.TEAM_ID=t.team_id  )team_name from 

(select player_id,player_name,(select player_role  from ipl_team_players ipltp where ipltp.PLAYER_ID=iplp.PLAYER_ID)player_role,
(select team_id  from ipl_team_players ipltp where ipltp.PLAYER_ID=iplp.PLAYER_ID)team_id,
 dense_rank()over(order by(cast(substr(performance_dtls,instr(performance_dtls,'wkt')+4,2) as float))desc)rnk 
from ipl_player iplp)t

where player_role in ('All-Rounder','Bowler') and rnk<=5
;
